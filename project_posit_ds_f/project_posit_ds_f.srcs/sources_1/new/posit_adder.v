...
module posit_adder (input1, input2, out, inf, zero);
...
wire s1 = input1[N-1];
wire s2 = input2[N-1];
wire zero_tmp1 = |input1[N-2:0];
wire zero_tmp2 = |input2[N-2:0];
wire inf1 = input1[N-1] & (~zero_tmp1),
	inf2 = input2[N-1] & (~zero_tmp2);
wire zero1 = ~(input1[N-1] | zero_tmp1),
	zero2 = ~(input2[N-1] | zero_tmp2);
...
wire [N-1:0] xin1 = s1 ? -input1 : input1;
wire [N-1:0] xin2 = s2 ? -input2 : input2;
